...
var websockets = require('/usr/local/lib/node_modules/socket.io'); // Use WebSockets
var http = require('http'); // Basic HTTP functionality
var path = require('path'); // Parse directory paths
var express = require('/usr/local/lib/node_modules/express'); // Provide static routing to pages
...
listen(function(socket)
{
	...
	socket.on('put-value',function(bpm)
	{
		console.log("value received from client: " + bpm.data);
		mbed.putResourceValue(endpoint,'3318/0/5900',bpm.data,function(err,data){
			if(err)
				throw err;
			socket.emit('put',{value:data});
			
		});
	});
	socket.on('reset-min-max',function()
	{
		mbed.postResource(endpoint,'3318/0/5605',"",function(err,data){
			if(err)
				throw err;
			socket.emit('reset',{value:data});
			console.log("Data received:" + data);
		});
	});
	socket.on('get-min',function()
	{
		mbed.getResourceValue(endpoint,'3318/0/5601',function(err,data){
			if(err)
				throw err;

			socket.emit('get-min',{value:data});
		});
	});
	socket.on('get-max',function()
	{
		mbed.getResourceValue(endpoint,'3318/0/5602',function(err,data){
			if(err)
				throw err;

			socket.emit('get-max',{value:data});
		});
	});
